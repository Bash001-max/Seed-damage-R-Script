...
# ==== Define directories ====
# Base directory on your DELL system
base_dir <- "C:/Users/DELL/Desktop/All_Days_analysis"
...
data_mean <- original.data %>%
  ...
  summarise(Mean = mean(nr_seeds_remov, na.rm = TRUE), .groups = "drop")

# ==== Calculate 95% confidence intervals ====
data.95 <- wsci(
  data = original.data, id = "id", dv = "nr_seeds_remov", 
  factors = c("day", "habitat", "species", "cage"),
  level = 0.95, method = "Morey"
)
...
print(unique(graph.data$species))   # Check species
print(unique(graph.data$cage))      # Check cage types
print(unique(graph.data$habitat))   # Check habitat levels

# Ensure habitat is factor with correct order
